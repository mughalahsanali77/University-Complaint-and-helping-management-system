...
public class EditDepartmentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            int dptId = Integer.parseInt(request.getParameter("id"));
            String dptName = request.getParameter("name");
            String dptDesc = request.getParameter("detail");
            int facultyId = Integer.parseInt(request.getParameter("facultyId"));

            out.println("<h1>" + dptId + "</h1>");
            out.println("<h1>" + dptName + "</h1>");
            out.println("<h1>" + dptDesc + "</h1>");

            FacultyDao fDao=new FacultyDao(FactoryProvider.getSessionFactory());
            Faculty faculty=fDao.getFacultyById(facultyId);
            
            DepartmentDao dDao = new DepartmentDao(FactoryProvider.getSessionFactory());
            Department department = dDao.getDepartmentById(dptId);
            department.setDepartmentName(dptName); 
            department.setDeparmentDescription(dptDesc);
            department.setFaculty(faculty);
            
            boolean f=dDao.updateDepartment(department);
              HttpSession session=request.getSession();
           if(f){
               
            session.setAttribute("message",department.getDepartmentName()+" updated Successfully");
            response.sendRedirect("view_departments.jsp");
           }else{
            session.setAttribute("message",department.getDepartmentName()+" unabled to update");
            response.sendRedirect("view_departments.jsp");
           }
