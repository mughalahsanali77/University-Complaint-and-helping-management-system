 ...
 * @author Ahsan
 */
public class RegisterSubjectsServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          int sid=  sDao.addSubject(subject);
           HttpSession session=request.getSession();
            if(sid>0){
            
                session.setAttribute("message","Subject Save SuucessFully Subject Name = "+ subject.getSubjectName() +" and id="+sid);
